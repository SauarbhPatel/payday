    ...
    public function makeAttendanceRow($attendance): array
    {
        return $attendance->details->map(function (AttendanceDetails $attendanceDetails) use ($attendance) {
            ...
            $total_hours = $attendanceDetails->out_time ?
                ...
                : '00:00';
            $break_time = $attendanceDetails->breakTimes->count() ?
                $this->convertSecondsToHoursMinutes($attendanceDetails->breakTimes->reduce(function ($carry, $break_time) {
                    return $carry + $this->carbon($break_time->start_at)->parse()->diffInSeconds($break_time->end_at);
                }, 0)) : '00:00';
            return $this->daily_log ? [
